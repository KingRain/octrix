...
export function ServiceTriangle({ service, isConnected = false }: ServiceTriangleProps) {
  ...
  const trianglePositions = useMemo(() => {
    const positions: { x: number; y: number; inverted: boolean }[] = [];
    const podSize = 18;
    ...
    const trianglesPerRow = Math.floor(containerWidth / (halfWidth + horizontalSpacing / 2));
    const maxRows = Math.floor(containerHeight / (triangleHeight + verticalGap));
    
    let podIndex = 0;
    
    for (let row = 0; row < maxRows && podIndex < displayPods.length; row++) {
      for (let col = 0; col < trianglesPerRow && podIndex < displayPods.length; col++) {
        const isInverted = col % 2 === 1;
        const x = col * (halfWidth + horizontalSpacing / 2);
        const y = row * (triangleHeight + verticalGap);
        
        positions.push({ x, y, inverted: isInverted });
        podIndex++;
      }
    }
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            <div className="relative" style={{ width: 160, height: 90 }}>
              ...
              >
                <path
                  d="M 12 0 L 155 0 L 143 90 L 0 90 Z"
                  className="transition-all duration-300"
                  style={{
                    fill: isHovered ? 'rgba(59, 130, 246, 0.08)' : 'rgba(30, 30, 30, 0.5)',
              ...
              <div className="absolute top-2 left-3" style={{ width: 140, height: 80 }}>
                {hasPods && (
                  <div className="relative">
                    {displayPods.map((pod, idx) => {
                      const pos = trianglePositions[idx];
                      if (!pos) return null;
                      ...
                      return (
                        <div
                          ...
                          style={{ 
                            left: pos.x,
                            top: pos.y,
                            transform: pos.inverted ? 'rotate(180deg)' : 'rotate(0deg)',
                          }}
                        >
                          <TrianglePod
                            name={pod.name}
        ...
        >
          <div className="px-4 py-3 border-b border-gray-700 bg-linear-to-r from-[#222] to-[#1a1a1a]">
            <p className="text-sm font-semibold text-white">{service.name.replace(/-/g, " ")}</p>
          ...
          <div className="px-4 py-3 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-sm text-green-400 font-semibold">{healthyCnt}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-sm text-yellow-400 font-semibold">{healingCnt}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-sm text-red-400 font-semibold">{failedCnt}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-sm text-blue-400 font-semibold">{service.pods.length}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-cyan-400 font-mono font-medium">{(service.totalCpu / 1000).toFixed(2)} cores</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-400">Memory Usage</span>
                <span className="text-xs text-purple-400 font-mono font-medium">{formatBytes(service.totalMemory)}</span>
              </div>
            </div>
          </div>
        </TooltipContent>
